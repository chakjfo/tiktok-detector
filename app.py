...
DATASET_CANDIDATES = [
    APP_DIR / "tiktok_accounts.csv",
    APP_DIR / "data" / "tiktok_accounts.csv",
    APP_DIR.parent / "datasets" / "tiktok_accounts.csv",
]
...
def load_raw_data() -> pd.DataFrame:
    for dataset_path in DATASET_CANDIDATES:
        if dataset_path.exists():
            return normalize_raw_dataset(pd.read_csv(dataset_path))

    checked_paths = "\n".join(f"- {path}" for path in DATASET_CANDIDATES)
    raise FileNotFoundError(
        "Could not find tiktok_accounts.csv. Upload it beside app.py or inside a data folder.\n"
        f"Checked:\n{checked_paths}"
    )
